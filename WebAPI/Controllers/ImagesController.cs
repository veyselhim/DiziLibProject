...
namespace WebAPI.Controllers
{
    ...
    public class ImagesController : ControllerBase
    {
        ...
        {
            var result = _imagesService.GetImagesByFilmId(id);
            if (result.Success)
            {
        ...
        [HttpGet("getimagesbyfilmid")]
        public IActionResult GetImagesById([FromForm(Name = ("CarId"))] int filmId)
        {
            var result = _imagesService.GetImagesByFilmId(filmId);
            if (result.Success)
            {
                return Ok(result);
            }
